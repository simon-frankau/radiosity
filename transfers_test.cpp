...
private:
    ...
    CPPUNIT_TEST(eachFaceIsAreaOneWithDifferentResolution);
    CPPUNIT_TEST_SUITE_END();

    void eachFaceIsAreaOne();
    void eachFaceIsAreaOneWithDifferentResolution();
};
...
void TransfersTestCase::eachFaceIsAreaOneWithDifferentResolution()
{
    TransferCalculator tc(cubeVertices, cubeFaces, RESOLUTION * 4);
    std::vector<double> sums = tc.calcSubtended();
    for (int i = 0; i < sums.size(); ++i) {
        CPPUNIT_ASSERT_DOUBLES_EQUAL(1.0, sums[i], 1.0e-6);
    }
